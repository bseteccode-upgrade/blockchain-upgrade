...
export class SlimScrollDirective implements OnInit, OnDestroy {
  ...
  getBarHeight(): void {
    ...
    if (this.isScrollStart || this.isScrollEnd) {
      this.barHeightCus = Math.max((this.el.offsetHeight / this.el.scrollHeight) * this.el.offsetHeight, 30);
    }
  ...
  scrollContent(y: number, isWheel: boolean, isJump: boolean): null | number {
    ...
    this.isScrollStart = isScrollAtStart;
    this.isScrollEnd = isScrollAtEnd;
  ...
  initWheel = () => {
    ...
    const wheelSubscription = merge(...[dommousescroll, mousewheel]).subscribe((e: MouseWheelEvent) => {
      ...
      this.previousValue = this.barHeightCus;
      if (this.currentValOffset == this.previousValue && ( this.isScrollStart || this.isScrollEnd)) {
        // window scroll
        return;
      } else {
        this.currentValOffset = this.previousValue;
        // stop window scroll
        if (e.preventDefault) {
          e.preventDefault();
        }
