/*
 * File : registersupplychain.component.ts
 * Use: register only supply chain user
...
@Component({
  selector: 'app-registersupplychain',
  templateUrl: './registersupplychain.component.html',
  styleUrls: ['./registersupplychain.component.css']
})
export class RegistersupplychainComponent implements OnInit {
  ...
  enterEmailSearch(emailAddress) {
    if (/^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/.test(emailAddress.toLowerCase())) {
      ...
      this.regForm.controls['register_type'].setValue('3');
